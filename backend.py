...
class Distancer:
    ...
    def insert(self,wo_id="",ma_id="",lm_user="",op="",count="",start_time="",end_time="",lm_time=""):
        self.conn = pymssql.connect(host='192.168.0.56',user='sa',password='1234',database='Raspberry_pi') 
    ...
    def detection_most_frequent_distance(self):
        List = []
    ...
    def detect_working(self,data,queue): 
        while True and not self.stop_flag:
            t_detect_1 = self.detection_most_frequent_distance()
            time.sleep(1)
            t_detect_2 =  self.detection_most_frequent_distance()
            time.sleep(1)
            t_detect_3 =  self.detection_most_frequent_distance()
            print("data d1: %dcm. d2: %dcm. d3: %dcm." %  (t_detect_1,t_detect_2,t_detect_3))
            # deal some unmoral issuses
